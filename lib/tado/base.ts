...
export class BaseTado {
  ...
  async #refreshToken(): Promise<void> {
    ...
    const shouldRefresh = this.accessToken.expired(
  ...
  resendInvitation(home_id: number, token: string): Promise<void> {
    return this.apiCall(
      `/api/v2/homes/${home_id}/invitations/${token}/resend`,
      "POST",
      {}
    );
  }
  ...
  deleteInvitation(home_id: number, token: string): Promise<void> {
    return this.apiCall(
      `/api/v2/homes/${home_id}/invitations/${token}`,
      "DELETE"
    );
  }
  ...
  async setGeoTracking(
    ...
    const settings = await this.getMobileDeviceSettings(
      home_id,
      mobile_device_id
    );
    return this.apiCall(
   ...
   */
  async getBoilerSystemInformation(
    system_id: number
  ): Promise<BoilerSystemInformation> {
